import { Check, Code, Database, Search, Settings, Zap } from "lucide-react";
import { useState } from "react";
import type { SectionProps, TechnicalComponent } from "./types";
  ...
  return (
    ...
    >
      ...
      <p className="text-gray-400 mb-6">
        The Architect supports a wide range of frameworks and technologies,
        letting you use the tools you already know and love.
      </p>
      ...
      <div className="flex flex-wrap gap-2 mb-8">
        {categories.map((category) => (
          <button
            key={category.id}
            type="button"
            onClick={() => setActiveCategory(category.id as "all" | "frontend" | "backend" | "fullstack")}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
        {frameworks.map((framework) => (
          <div
            key={framework.id}
            className={
              "bg-gray-800 rounded-lg p-4 flex flex-col items-center justify-center border border-gray-700 hover:border-purple-500 transition-all hover:scale-105 group cursor-pointer h-32"
            }
          >
            <div
              className={`w-12 h-12 rounded-full ${framework.color} flex items-center justify-center mb-3`}
            >
