...
export default function FrameworkCard({
  ...
  return (
    <Card
      ...
      className={cn("transition-all duration-300", expanded && "animate-pulse")}
    >
          ...
          <div className="mt-4 text-sm space-y-3 border-t pt-3 animate-slideDown">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1 stagger-children">
                {framework.compatible_modules?.map((moduleId) => (
                  <span
                    key={moduleId}
                    className="badge badge-outline badge-sm animate-fadeIn"
                  >
                    {moduleId}
                  </span>
                ))}
                {(!framework.compatible_modules ||
                  framework.compatible_modules.length === 0) && (
                  <span className="text-xs text-base-content/50">
                    No compatible modules specified
                  </span>
                )}
              </div>
            </div>
