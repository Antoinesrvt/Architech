  ...
  return (
    <div 
      className={cn(
        ...
        isHovering && !disabled && !selected ? "transform -translate-y-1 shadow-md" : "",
        expanded ? "z-10" : "",
        "animate-fadeIn"
      )}
      ...
      <div className="card-body p-4">
        <div className="flex items-start justify-between">
          <div className="flex items-center">
            <div className="relative mr-3">
              ...
              {!disabled && !selected && isHovering && (
                <span className="absolute inset-0 w-full h-full bg-secondary/20 rounded-md animate-pulse"></span>
              )}
            </div>
            <div>
              <h3 className="text-base font-semibold leading-tight">{module.name} <span className="text-xs opacity-70">v{module.version}</span></h3>
          ...
          <div className="flex gap-2">
            ...
            >
              {expanded ? (
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 transition-transform duration-300 -translate-y-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
