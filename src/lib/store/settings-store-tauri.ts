...
export const useSettingsStore = create<SettingsState>((set, get) => ({
  ...
  loadSettings: async () => {
    try {
      ...
      const theme = await storeInstance.get<ThemeType>('theme') ?? defaultSettings.theme;
      const defaultProjectPath = await storeInstance.get<string>('defaultProjectPath') ?? defaultSettings.defaultProjectPath;
      const editorCommand = await storeInstance.get<string>('editorCommand') ?? defaultSettings.editorCommand;
      const autoOpenProjectAfterGeneration = await storeInstance.get<boolean>('autoOpenProjectAfterGeneration') ?? defaultSettings.autoOpenProjectAfterGeneration;
      const useGit = await storeInstance.get<boolean>('useGit') ?? defaultSettings.useGit;
